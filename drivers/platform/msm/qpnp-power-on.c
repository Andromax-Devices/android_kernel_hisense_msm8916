...
#define QPNP_PON_HARD_RESET_BIT			BIT(0)
...
int qpnp_pon_is_hard_reset(void)
{
	struct qpnp_pon *pon = sys_reset_dev;
	int rc;
	u8 reg;
	...
	if (reg & QPNP_PON_HARD_RESET_BIT)
		return 1;


	return 0;
}
EXPORT_SYMBOL(qpnp_pon_is_hard_reset);
...
extern int hs_is_built_in_battery(void);
...
static int qpnp_config_reset_for_hs(u32 pon_type, u32 s2_type, u32 s1_timer, u32 s2_timer, u32 enable)
{
	int rc;
	u8 i;
	struct qpnp_pon *pon = sys_reset_dev;
	...
	printk(KERN_INFO "%s: pon_type %d, s2_type %d, s1_timer %d, s2_timer %d, enable %d\n", 
		__FUNCTION__, pon_type, s2_type, s1_timer, s2_timer, enable);

	if (NULL == pon)
	{
		printk(KERN_ERR "%s: pon device not init!\n", 
			__FUNCTION__);
		return -1;
	}
	...
	{
		printk(KERN_ERR "%s: Cant find pon config!\n", __FUNCTION__);
		return 1;
	}
...
void hs_set_pm_pon_reset(int debug)
{
	
	printk(KERN_INFO "%s: debug %d.\n", __FUNCTION__, debug);
	if (hs_is_built_in_battery())
	{
#ifdef CONFIG_MSM_RELEASE_VERSION
		if (debug)
		{
		...
		{
			qpnp_config_reset_for_hs(PON_RESIN, 
				PM_PON_RESET_CFG_NORMAL_SHUTDOWN, 10256, 2000, 1);
		}
#endif	/*#ifdef CONFIG_MSM_RELEASE_VERSION*/
	}
	else
