...
static struct subsys_device *g_my_subsys[5] = {NULL};

void set_subsys_restart_level(int restart_level)
{
	int i = 0;

	printk(KERN_INFO "%s: restart_level %d.\n", __FUNCTION__, restart_level);
	if ((restart_level != RESET_SUBSYS_COUPLED) 
		&& (restart_level != RESET_SOC))
	{
		printk(KERN_ERR "%s: Invalid restart_level %d!\n", 
			__FUNCTION__, restart_level);
		return;
	}

	for (i = 0; i < ARRAY_SIZE(g_my_subsys); i++)
	{
		if (NULL !=  g_my_subsys[i])
		{
			printk(KERN_INFO "%s: Set subsys name %s, restart level: %d\n", 
				__FUNCTION__, g_my_subsys[i]->desc->name, restart_level);
			g_my_subsys[i]->restart_level = restart_level;
		}
	}
...
#ifdef CONFIG_HS_PANIC_HANDLE
#ifdef CONFIG_MSM_RELEASE_VERSION	//add by duxiaojie
			subsys->restart_level = RESET_SUBSYS_COUPLED;;
#else   /*#ifdef CONFIG_MSM_RELEASE_VERSION*/
			subsys->restart_level = RESET_SOC;
#endif  /*#ifdef CONFIG_MSM_RELEASE_VERSION*/

	if (subsys->id > ARRAY_SIZE(g_my_subsys))
	{
		printk(KERN_ERR "subsys id exceed sizeof(g_my_subsys)!\n");
	}
	else
	{
		printk(KERN_INFO "Add subsys ID %d, name %s\n", 
			subsys->id, subsys->desc->name);
		g_my_subsys[subsys->id] = subsys;
	}
#endif	/*CONFIG_HS_PANIC_HANDLE*/
